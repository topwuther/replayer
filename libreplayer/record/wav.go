...
func NewFile(w io.ReadWriteSeeker, sampleRate, channels int) *File {
...
func (f *File) Read(p []float32) (int, error) {
	return len(p), binary.Read(f.out, binary.LittleEndian, p)
}
